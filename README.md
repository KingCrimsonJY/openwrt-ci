...
本仓库还包含以下独立功能：
- **独立的插件构建**: `build-packages.yml` 专门用于构建 `luci-app-daed` 等大型插件，独立于固件发布。
- **自动更新检查**: `update-checker.yml` 定时检查上游源码更新。
...
6. 编译大概需要3-5小时，编译完成后在仓库主页 [Releases](https://github.com/KingCrimsonJY/openwrt-ci/releases) 对应 Tag 标签内下载固件
